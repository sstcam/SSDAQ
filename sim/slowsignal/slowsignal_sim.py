...
class TMSimulator(object):
    ...
    async def send_ss_data(self):
        ...
        while True:
            ...
            if counter == packets_per_sec:
                counter = 0
                master_tmstmp = int(
                    self.t1.timestamp() + self.dt
                )  # +1#int(self.t1.timestamp())
            #     t = datetime.utcnow()
            #     sleeptime = (1.0 - t.microsecond/1000000.0)
            ...
            if counter == 0:
                ...
                timestamp_packet = (
                    master_tmstmp + cloc_cycles[counter]
                )  # + np.random.uniform(-2e-5,2e-5)
            else:
            ...
            for i in range(10):
                ...
                data_packet.extend(
                    struct.pack(">Q", int((timestamp_packet + 0.1 * self.dt * i) * 1e9))
                )
                data_packet.extend(struct.pack(">32H", *data[:32]))
                data_packet.extend(
                    struct.pack(
                        ">Q", int((self.t1.timestamp() + 0.1 * self.dt * i) * 1e9)
                    )
                )
                data_packet.extend(struct.pack(">32H", *data[32:]))
                self.nreading += 1
            ...
            if self.npackets % 1 == 0:
                ...
                rate = self.npackets_c / float(
                    dtt.seconds + dtt.microseconds / 1000000.0
                )
                actual_dt = 1 / rate
                print("Rate %g" % (rate))
                self.npackets_c = 0
            ...
            t2 = datetime.utcnow()
            sleeptime = (
                -(t2 - self.t1).microseconds / 1000000.0 + self.dt
            )  # *self.dt/actual_dt
    ...
    def _simulate_data(self):
        ...
        data_counts = np.asarray(self.data / (0.03815 * 2.0), dtype=np.uint16)
