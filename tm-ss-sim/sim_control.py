...
class SimControl(object):
    def __init__(self,subprogram_list):
        ...
        self.parser = argparse.ArgumentParser(description='Communicate with TM simulation processes to change the simulation dynamically',
...
class StatusQuery(object):
    def __init__(self, name = 'status' ):
        self.name = name
        self.queries_map = {'npackets':b'get_npackets_sent',
                            'ss-rate':b'get_rate',
                            'sending':b'is_sending_ss_data'}

    def init_parser(self,cmd_parser):
        self.status_parser = cmd_parser.add_parser('status',
                                                    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
